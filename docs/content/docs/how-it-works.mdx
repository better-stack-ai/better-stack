---
title: How It Works
description: Understand the architecture and key concepts behind Better Stack.
---

Here's a high-level overview of how Better Stack works:
...
- **DB Adapter**: Translates Better Stack's database operations to your ORM (Prisma, Drizzle, Kysely, MongoDB). Plugins define schemas that get merged and passed to the adapter.
...
The `BetterStackProvider` wraps your pages and injects framework-specific components via React Context. Plugin components access these overrides through `usePluginOverrides()`, allowing them to use your framework's `Link`, `Image`, and navigation without tight coupling and to avoid breaking the client/server boundary in frameworks like Next.js.

## Plugins

Plugins are the building blocks of Better Stack. Each feature (like Blog) ships as **two separate plugins**—one for the backend, one for the client—that you register independently:
...
**Backend plugins** (registered in `betterStack`):
