...
Better Stack uses [Better DB (`@btst/db`)](https://github.com/better-stack-ai/better-auth/tree/main/packages/btst) as its database abstraction layer—a specialized fork of [better-auth](https://www.better-auth.com)'s database layer optimized for Better Stack's plugin architecture.
