...
### RC Backend Hooks and Lifecycle

Existing plugins may retain hook-based authorization while their operations are
migrated. New schema-backed operations authorize before lifecycle hooks run.
Treat migrated `onBefore*` hooks as post-authorization domain invariants; they
cannot observe or replace validation, fact derivation, identity, or
authorization failures.

```typescript
export interface BlogBackendHooks {
  // RC compatibility authorization hooks for unmigrated operations
  onBeforeCreatePost?: (data, context) => Promise<void> | void
  onBeforeUpdatePost?: (postId, data, context) => Promise<void> | void
  // Migrated delete lifecycle: server authorization already succeeded
...
### Raw Server-side Mutations (`mutations.ts`)

Plugins may keep low-level **write primitives** that bypass the operation layer
for migrations, test setup, and seed scripts. Runtime application writes should
use schema-backed operations through `forRequest(request).api` or the explicit
trusted `internal` namespace so validation and lifecycle behavior stay intact.
...
<Callout type="warn">
**Raw mutation functions bypass authorization and lifecycle hooks.** Never call
them from user-facing request handlers. Prefer a schema-backed operation for
runtime work, including AI tools and trusted jobs.
</Callout>
...
- `stack().api.todos.*` surface for read-only and legacy raw server access
