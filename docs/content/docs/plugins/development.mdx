...
export const todosBackendPlugin = defineBackendPlugin({
  ...
  routes: (_adapter, _context, operations) => ({
    deleteTodo: operations.deleteTodo.route(
      createEndpoint(
        "/todos/:id",
        { method: "DELETE", requireRequest: true },
        (ctx) => operations.deleteTodo({ id: ctx.params.id }, ctx.request),
      ),
    ),
  }),
})
...
same-key operation and bind its endpoint with
`operations.operationKey.route(endpoint)`. The binding carries a private marker
from the request-bound operation transport; `stack()` validates both the
inventory and the exact binding during composition. It reports the plugin key,
route key, method, and path for an undeclared or unbound route. This keeps a new
route from silently bypassing the operation pipeline. Extra operations may
remain request/internal-only and are not exposed as HTTP merely because they
exist in the operation registry.
