...
`execute` runs. The callback may load trusted server state to build those
requests, but it must not mutate it. Trusted `internal` execution still derives
the requests and runs the lifecycle; it skips only user authorization.
