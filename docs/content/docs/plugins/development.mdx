...
```typescript
import { joinBasePath, useBasePath, useStack } from "@btst/stack/context"
...
function TodosList() {
  ...
  return (
    <Link href={joinBasePath(basePath, "/todos/add")}>
      Add Todo
    </Link>
  )
}
...
Use `joinBasePath` for links and programmatic navigation. It keeps a root base path (`/`) from producing a protocol-relative URL such as `//todos/add`.
