...
request authorship when server authorization is configured. Without server
authorization, an explicit `authorId` input remains available for trusted and
...
Comments has no raw `stack.api.comments` business namespace. Use `app.internal.comments` for explicitly trusted jobs that should keep operation validation and lifecycle hooks. Standalone getter exports remain lower-level adapter primitives for plugin internals and migrations.
