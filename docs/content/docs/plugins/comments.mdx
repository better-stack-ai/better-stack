...
| `onBeforePost` | hook | — | Called before a comment is saved. Throw to reject. **Required for security** — see callout below. |
...
<Callout type="warn">
**`onBeforePost` is the only server-side identity gate for comment creation.** The `authorId` field is passed in the request body — without `onBeforePost`, any caller can post comments attributed to any user ID. Always verify that `input.authorId` matches the authenticated session (e.g. check a JWT or session cookie) and throw if it does not.

If `onBeforePost` is not configured, the plugin emits a `console.warn` at startup.
</Callout>
