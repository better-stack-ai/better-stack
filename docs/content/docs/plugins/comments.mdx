  ...
  plugins: {
    comments: commentsBackendPlugin({
      ...
      onBeforeDelete: async (commentId, ctx) => {
        // Keep this aligned with the client comments:comment/delete permission
        const session = await getSession(ctx.headers)
        if (!session?.user?.isAdmin) throw new Error("Admin access required")
      },
...
<StackProvider<PluginOverrides>
  ...
  auth={{
    ...
    loginPath: "/login",
    can: ({ resource, action }) => {
      if (resource !== "comments:comment") return true
      if (action === "moderate" || action === "delete") return isAdmin()
      return true
    },
  }}
  overrides={{
...
For comments owned by the current user, `CommentThread` renders the Delete
button only when `StackProvider.auth.can` allows
`{ resource: "comments:comment", action: "delete", params: { id } }`. Keep this
client permission aligned with `onBeforeDelete`; the server hook remains the
authoritative security check. If the server still rejects a delete, the thread
reports the failure through the configured `notify.error` handler.
...
Access is controlled by the `comments:comment` / `moderate` permission through `StackProvider.auth.can`.
...
- `resolveCurrentUserId` must return the session-verified user ID (or `null` when unauthenticated) — any caller-supplied `?currentUserId=…` query parameter is completely discarded.
</Callout>
