...
export const authorization = defineAuthorization({
  ...
  rules: ({ comments }) => [
    ...
    comments.comment.moderate.when(({ identity }) =>
...
Access is controlled by `comments.thread.read({ scope: "moderation", ... })`;
each row and bulk action additionally evaluates exact `comment.moderate` or
`comment.delete` facts.
