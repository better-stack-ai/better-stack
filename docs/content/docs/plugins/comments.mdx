...
the authorization migration. Supplying a structural legacy server auth
provider is different: operation-backed endpoints fail closed and require a
provider created with `createServerAuth()`. The legacy string client provider
also remains narrowly supported during that migration; explicitly public
approved threads/counts stay public, while schema-bound providers always
