...
| `onBeforePost` | hook | **required** | Called before a comment is saved. Must return `{ authorId: string }` derived from the authenticated session. Throw to reject. Plugin throws at startup if absent. |
...
<Callout type="warn">
**`onBeforePost` is required and must return `{ authorId: string }`.** `authorId` is intentionally absent from the POST body schema — the server derives identity exclusively from this hook. Resolve `authorId` from the session (e.g. JWT / session cookie) and throw if the user is not authenticated. A client can never forge comment authorship.
...
The comments plugin registers a `/comments/my-comments` route that shows the current user's full comment history — all statuses (approved, pending, spam) in a single paginated table, newest first.

**Features:**
- All comment statuses visible to the owner in one list, each with an inline status badge
- Prev / Next pagination (20 per page)
- Resource link column — click through to the original resource when `resourceLinks` is configured
...
| `currentUserId` | `string \| (() => string \| undefined \| Promise<string \| undefined>)` | Authenticated user's ID — used by the My Comments page. Supports async functions for session-based resolution. |
| `resourceLinks` | `Record<string, (id: string) => string>` | Per-resource-type URL builders for linking comments back to their resource on the My Comments page (e.g. `{ "blog-post": (slug) => "/pages/blog/" + slug }`). |
...
| `onBeforeMyCommentsPageRendered` | hook | Called before rendering the My Comments page. Throw to deny access (e.g. when no session exists). |
