...
Inside `StackProvider`, `useRegisteredRoutes()` is automatically bound to that
...
Then pass the ejected page back to the client plugin. Route Docs remains
client-only; do not add a matching backend plugin.
