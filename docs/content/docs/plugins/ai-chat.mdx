...
Configure framework-wide router and API wiring on `StackProvider`; keep only AI Chat-specific values in `overrides`:
...
The streaming operation checks `stream.start` plus the exact semantic intent (`message.send`, `message.edit`, `message.retry`, attachment, tool, and conversation creation when applicable) before hooks, persistence, or provider work. A coarse stream rule cannot bypass a denied sub-operation rule.
...
```ts
await app.forRequest(request).api.aiChat.deleteConversation({ id }) // authorized
await app.internal.aiChat.deleteConversation({ id })                // trusted
```

`internal` skips only user authorization. Input validation, authoritative reads, lifecycle hooks, persistence, and provider/tool behavior remain active. The raw getters documented below deliberately bypass authorization and lifecycle composition.
  ...
  return (
    ...
    <div className="fixed inset-0 z-50 bg-black/50" onClick={() => router.back()}>
      ...
      <div className="..." onClick={(e) => e.stopPropagation()}>
        <StackProvider api={{ baseURL, basePath: "/api/data" }} ...>
...
The stable prefixes remain `['conversations', 'list', 'all']` and `['conversations', 'detail', id]`. Protected keys append only the validated identity id (or an explicit anonymous marker), never the full identity object. Pending/error identity generations use separate disabled keys, mutations refresh only the partition that started them, and an account switch stops and clears an active stream. Server loaders must pass `identityPartition` (the framework codegen does this automatically) so dehydrated data lands in the same browser partition.
...
The AI Chat plugin exposes standalone getter functions for deliberately low-level server data access.

<Callout type="warn">
These raw getters bypass typed authorization, identity scoping, authoritative snapshot checks, and lifecycle hooks. Prefer `app.forRequest(request).api.aiChat` for request-driven code and `app.internal.aiChat` for trusted jobs. Use raw getters only when the caller owns those responsibilities explicitly.
</Callout>
