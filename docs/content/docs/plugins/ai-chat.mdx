      ...
      return (
        <QueryClientProvider client={queryClient}>
          <StackProvider<PluginOverrides>
            ...
            overrides={{
              "ai-chat": {
                mode: "authenticated", // Persistent conversation UI
...
AI Chat has no raw `stack.api.aiChat` business namespace. Use `app.forRequest(request).api.aiChat` for request-driven work and `app.internal.aiChat` for explicitly trusted jobs. Standalone conversation getters remain lower-level adapter primitives for plugin internals and migrations.

## Route-Aware AI Context## Route-Aware AI Context
