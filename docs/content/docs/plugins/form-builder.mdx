...
| `useSubmissions(formId)` | List submissions and authoritative form facts | `{ form, submissions, total, isLoading }` |
...
not expose the form schema or other fields protected by `form.read`.
...
| `/form-builder/forms/:formId/submissions` | GET | List submissions for a form |
| `/form-builder/forms/:formId/submissions/:subId` | GET | Get a submission |
...
| `onBeforeFormCreated` | `data, ctx` | `data \| void` | Validate/transform before create; throw to deny |
...
| `onBeforeFormUpdated` | `id, data, ctx` | `data \| void` | Validate/transform before update; throw to deny |
...
| `onBeforeSubmission` | `slug, data, ctx` | `data \| void` | Validate/rate limit submissions; throw to deny |
...
The Form Builder plugin exposes standalone getter functions for trusted
server-side and build-time use cases. These raw helpers bypass operation
authorization and lifecycle hooks. Prefer `forRequest(request).api.formBuilder`
