...
Configure the shared client runtime once, then give Media only its own upload
...
export function ClientLayout({ children, initialIdentity }) {
...
This preserves Media endpoint overrides, public browser headers, explicit
credentials, and the configured upload mode. The generated apps use the same
pattern to connect Blog, CMS, and Kanban image uploads to Media.
