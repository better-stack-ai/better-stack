...
UI Builder is a CMS HTTP consumer, so it inherits the CMS plugin's effective API
origin, path, browser headers, and credentials. Configure a CMS data replacement
once with `endpoints.cms.api`; `endpoints.uiBuilder` may customize only the UI
site location.

### 3. Configure the Provider

Pass the resolved stack to the provider. The component registry is a
`uiBuilderClientPlugin()` concern, so `StackProvider` needs no duplicate override:
...
UI Builder is intentionally client-only over CMS. The `cmsBackendPlugin`
supplies its persistence and HTTP contract; there is no UI Builder backend
plugin to register.
...
// Register once; the editor and PageRenderer consume the same registry.
const uiBuilder = uiBuilderClientPlugin({ components: customRegistry })
```
...
| `componentRegistry` | `ComponentRegistry` | Standalone override; otherwise uses the plugin factory registry, then the built-in default |
...
`onErrorLoad` is reporting-only: callback errors are contained and the loader
never rejects, so it cannot perform throwing framework redirects:
...
| `onErrorLoad` | `error, context` | `void` | Report loader failures; callback errors are contained |
...
#### UIBuilderClientConfig

<AutoTypeTable path="../packages/stack/src/plugins/ui-builder/client/plugin.tsx" name="UIBuilderClientConfig" />

Pass custom component definitions through the `components` factory field. An
explicit `PageRenderer` `componentRegistry` prop is a standalone-renderer
override and takes precedence; otherwise the registered factory value, then the
built-in registry, is used.
