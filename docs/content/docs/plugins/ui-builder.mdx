...
| UI | Permission |
|----|------------|
| Page list route | `cms.record.read({ contentType: UI_BUILDER_TYPE_SLUG })` |
...
`UI_BUILDER_TYPE_SLUG` and `recordId` is present. A condition on the content
type alone also makes the admin page list public. `PageRenderer` uses the same
CMS descriptor as the backend read; its browser check is only a presentation
gate. The raw SSG getters/prefetch helpers are trusted lower-level APIs and must
only be called from server/build code.
</Callout>
...
| Public `PageRenderer` | `cms.record.read({ contentType, recordId, authorId? })` |
...
export const authorization = defineAuthorization({
  ...
  rules: ({ cms }) => [
    ...
    cms.record.read.when(
      ({ identity, facts }) =>
        (facts.contentType === UI_BUILDER_TYPE_SLUG && facts.recordId !== undefined) ||
        identity?.role === "admin",
    ),
...
The `recordId` condition makes a by-slug page render public without making the
admin page list public. For a by-slug read, the CMS backend finds the matching
record and derives its ID and ownership before rule evaluation.
