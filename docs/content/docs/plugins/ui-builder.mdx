...
`UI_BUILDER_TYPE_SLUG` and `scope` is `"record"`. A condition on the content
...
| Public `PageRenderer` | `cms.record.read({ contentType, scope: "record", recordId, authorId? })` |
...
The `scope` condition makes a by-slug page render, including its not-found
