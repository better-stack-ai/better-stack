...
| `cms.record.read` | `contentType`, `recordId?`, `authorId?` | List, get, populate, and relation reads |
...
`contentType`, `recordId`, and `authorId`; client-supplied ownership or type
claims cannot authorize a write. Rules are boolean operation checks. If your
