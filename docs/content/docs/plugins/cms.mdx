...
interfaces in #193. Compound inline-create, populated-record, and inverse
relation checks map every server-derived target or source separately, so a
...
relation rows atomically. Trusted `internal.cms` calls skip those user checks
but keep the same validation and transaction lifecycle.
