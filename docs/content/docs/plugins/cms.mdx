...
`myStack.api.cms` contains only the SSG `prefetchForRoute` helper. Use `forRequest(request).api.cms` for request work and `internal.cms` for explicitly trusted jobs; both retain validation and lifecycle behavior.
...
```ts
const types = await myStack.internal.cms.listContentTypes({})
const items = await myStack.internal.cms.listContentItems({
  typeSlug: "posts",
  query: { limit: 10 },
})
await myStack.forRequest(request).api.cms.createContentItem({
  typeSlug: "client-profile",
  body: { slug, data },
})
```

`forRequest(request).api.cms` enforces authorization. `internal.cms` is the explicit trusted surface and still runs validation, authoritative fact derivation, relation planning, transactions, and lifecycle hooks. `myStack.api.cms` is reserved for `prefetchForRoute`.
