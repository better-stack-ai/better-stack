...
<Callout type="info">
**Why configure API paths here?** This configuration is used by **server-side loaders** that run outside React Context. Client-side components use the top-level `StackProvider.api` configuration shown in Section 4.
</Callout>
...
Configure framework-wide router and API wiring on `StackProvider`; keep only Blog-specific values in `overrides`:
...
For migration compatibility, the string-based RC client provider keeps the
previously public published-list, published-detail, and tag routes open. Its
drafts route keeps its former `{ resource: "blog:draft", action: "read" }`
permission, while the temporary descriptor-to-string mapping applies to
mutation controls. Schema-bound one-rule providers always evaluate the exact
descriptor, so the explicit public rules above remain the source of truth.
...
const blogHooks: BlogBackendHooks = {
  ...
  async onPostUpdated(post, context) {
...
The Blog plugin exposes standalone getters for trusted server reads and raw
mutation primitives for build-time and administrative work. Raw functions
...
When `stack()` receives a one-rule server adapter, use the request-scoped API
...
```ts
const blog = myStack.forRequest(request).api.blog;
...
```ts
await myStack.internal.blog.updatePost({
  id: postId,
  data: update,
});
```

`internal` skips identity resolution and user authorization only. It is the
appropriate surface for a trusted job that still needs normal Blog behavior.

### Raw data access patterns

**Pattern 1 — via `stack().api` (lower-level raw access)**

After calling `stack()`, the returned object includes a fully-typed `api`
namespace. Getters are pre-bound to the adapter and are useful for trusted
read-only server work:
...
Do not use raw `myStack.api.blog.createPost()`, `updatePost()`, or
`deletePost()` in request handlers. Use `forRequest(request).api.blog.*` for
user-facing work or `myStack.internal.blog.*` for explicitly trusted jobs.
...
<Callout type="warn">
**No authorization or lifecycle hooks are called** when using raw
`stack().api.*` mutations or direct imports. These functions hit the database
directly. Keep them to migrations, seed scripts, and other explicitly trusted
low-level work; never call them from user-facing request handlers.
</Callout>
...
<Callout type="warn">
`prefetchForRoute()` is a trusted raw-data API. It does not resolve identity,
