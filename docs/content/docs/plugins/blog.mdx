...
The `blogBackendPlugin()` accepts optional compatibility and lifecycle hooks for
customizing domain behavior, logging, and unmigrated RC authorization.
...
The Blog plugin publishes the schema-backed `blogPermissions.post.delete`
descriptor. The new one-rule path uses it for local, type-safe UI gates and
derives its trusted `{ id, authorId? }` facts again on the server before the
delete mutation. See the [authorization guide](/auth) for the shared client and
server setup.

Unmigrated Blog controls continue to use the RC checks
`blog:draft/read`, `blog:post/create`, `blog:post/update`, and
`blog:post/delete`. Configure `StackProvider.auth` for those controls and use
their compatibility hooks for authoritative API authorization. The migrated
delete operation authorizes before any delete lifecycle hook runs.
...
Customize backend behavior with optional compatibility and lifecycle hooks.
Unmigrated operations retain their RC authorization hooks. The migrated delete
hooks run only after the shared server rule allows the operation, so use them
for domain invariants, logging, and side effects—not ordinary access control.
...
const blogHooks: BlogBackendHooks = {
  // RC compatibility authorization hooks for operations not migrated yet.
  ...
  // Delete is migrated: identity and trusted facts are already authorized.
...
#### Blog delete lifecycle contexts

The migrated delete operation supplies validated input, server-derived facts,
resolved identity, and request directly to its lifecycle hooks. The result is
also available after execution, and the error context is only created after
authorization succeeds. Validated input, trusted facts, identity, and results
are plain lifecycle data that is deeply readonly and frozen before hooks can
observe it.
...
mutation primitives for RC compatibility. Raw functions bypass the operation
layer entirely. Keep raw mutations to migrations, test setup, and seed scripts;
use the authorized operation API below for runtime deletion.

### Authorized delete operation

When `stack()` receives a one-rule server adapter, use the request-scoped API
for user-facing Blog deletion. It runs the same operation as the HTTP DELETE
endpoint, including input validation, trusted fact derivation, authorization,
and lifecycle hooks:

```ts
await myStack
  .forRequest(request)
  .api.blog.deletePost({ id: postId });
```
...
The existing `myStack.api.blog.deletePost(postId)` and direct mutation import
remain raw RC escape hatches for compatibility. They do not run authorization
or lifecycle hooks.

### Raw data access patterns

**Pattern 1 — via `stack().api` (read-only and legacy raw access)**
...
Do not use raw `myStack.api.blog.deletePost()` in request handlers. Use
`forRequest(request).api.blog.deletePost()` for user-facing deletion or
`myStack.internal.blog.deletePost()` for explicitly trusted jobs. The raw
mutation remains only for RC compatibility while the other Blog mutations are
migrated.
