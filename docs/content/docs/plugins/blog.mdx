...
Use `myStack.internal.blog` for trusted jobs that should keep normal validation, fact derivation, domain behavior, and hooks. Use `myStack.forRequest(request).api.blog` for user-driven server work. `myStack.api.blog` contains only `prefetchForRoute`.
...
export async function generateStaticParams() {
  const { items } = await myStack.internal.blog.listPosts({ published: true, limit: 1000 })
