...
identity is unresolved, each auth-provider/hydration source gets its own pending
generation, so a new login flow cannot reuse a prior user's pending response.
