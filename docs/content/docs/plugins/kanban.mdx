...
Omitting stack authorization preserves permissive RC compatibility during the
v3 migration. When authorization is configured, ordinary anonymous denials are
...
HTTP endpoints, request-scoped calls, and trusted internal calls all adapt these
same operations.
...
```ts
await app.forRequest(request).api.kanban.updateTask({
  id: taskId,
  data: { title: "Ready for review" },
})
```

Trusted jobs, seeds, and tests use `internal`. This skips only user identity and
...
```ts
await app.internal.kanban.moveTask({
  taskId,
  targetColumnId,
  targetOrder: 0,
})
...
inventory. Raw getters, mutations, and SSG helpers are available separately on
`app.api.kanban` and as direct imports; they bypass operation authorization and
...
| Group | Hooks |
|---|---|
| Board before | `onBeforeListBoards`, `onBeforeReadBoard`, `onBeforeCreateBoard`, `onBeforeUpdateBoard`, `onBeforeDeleteBoard` |
| Board after | `onBoardsRead`, `onBoardRead`, `onBoardCreated`, `onBoardUpdated`, `onBoardDeleted` |
| Board error | `onListBoardsError`, `onReadBoardError`, `onCreateBoardError`, `onUpdateBoardError`, `onDeleteBoardError` |
| Column before | `onBeforeCreateColumn`, `onBeforeUpdateColumn`, `onBeforeDeleteColumn` |
| Column after | `onColumnCreated`, `onColumnUpdated`, `onColumnDeleted` |
| Task before | `onBeforeCreateTask`, `onBeforeUpdateTask`, `onBeforeDeleteTask` |
| Task after | `onTaskCreated`, `onTaskUpdated`, `onTaskDeleted` |
...
`useBoards()`, `onBoardsRead`, and `afterLoadBoards` now receive
...
The Kanban plugin exposes standalone getter functions for trusted server-side
and build-time use cases. These helpers and `app.api.kanban` are deliberately
raw: they bypass operation authorization and lifecycle hooks. Prefer
`app.forRequest(request).api.kanban` for any user-driven request.

The authorized `listBoards` operation returns board/column summaries without
task rows. Fetching one authorized board returns its complete column/task tree.
The raw `getAllBoards` helper below retains its lower-level full-tree behavior.
...
<Callout type="warn">
These raw mutation functions bypass both operation authorization and lifecycle
hooks. Prefer `app.internal.kanban` for trusted work that should preserve
validation, ordering invariants, and hooks. Use a raw mutation only when the
caller deliberately owns all those responsibilities; never expose one directly
from a user-facing request handler.
</Callout>
