...
<Callout type="warning">
Authorization-sensitive Kanban writes require an adapter with real isolated
transactions. The ordinary memory adapter and a sequential transaction fallback
fail closed because they cannot keep the server-derived authorization snapshot
stable across hooks and the write. Use a transactional production adapter; a
serialized wrapper is suitable only for single-process tests.
</Callout>
