...
inventory. The raw `app.api.kanban` business surface was removed. It now contains only `prefetchForRoute`; use `internal.kanban` for explicitly trusted business calls.
...
Use `app.forRequest(request).api.kanban` for user-driven server calls and `app.internal.kanban` for explicitly trusted jobs. The authorized list operation returns board/column summaries; a single-board read returns its complete tree. `app.api.kanban` is reserved for SSG `prefetchForRoute`.
...
Trusted jobs call the maintained internal operations:

```ts
const board = await app.internal.kanban.createBoard({ name: "Review Queue" })
await app.internal.kanban.createTask({ title: "Review", columnId })
```
