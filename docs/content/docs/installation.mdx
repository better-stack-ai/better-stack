 ...
 <Tabs items={["prisma", "drizzle", "kysely"]}>
  ...
  <Step>
    ...
    <Callout type="warn">
      ...
      `resolveTrustedServerOrigin` from `@btst/stack/client/server`, fails
  ...
  <Step>
    ...
    <Tabs groupId="frameworks" items={["next-js", "react-router", "tanstack"]} persist>
      ...
      <Tab value="tanstack">
        ...
        const getClientOrigins = createIsomorphicFn()
          ...
          .client(() => ({
            apiOrigin: import.meta.env.VITE_PUBLIC_API_URL || window.location.origin,
            siteOrigin: import.meta.env.VITE_PUBLIC_SITE_URL || window.location.origin,
          }))
    ...
    so the server provider and hydrated browser stack resolve the same endpoints; keep
    request headers and server stack instances out of loader data.
  </Step>
