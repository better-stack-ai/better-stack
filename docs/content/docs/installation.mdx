...
<Steps>
 ...
 <Tabs items={["prisma", "drizzle", "kysely"]}>
    ...
    export const getStackClient = (
      ...
      const baseURL =
        typeof window === "undefined"
          ? options?.origin || process.env.BASE_URL || "http://localhost:3000"
  ...
  <Step>
    ...
    <Tabs groupId="frameworks" items={["next-js", "react-router", "tanstack"]} persist>
      ...
      <Tab value="tanstack">
        ...
        export const Route = createFileRoute("/pages")({
          loader: async () => ({ requestOrigin: await getRequestOrigin() }),
          component: PagesLayout,
        })

        function PagesLayout() {
          const { queryClient } = Route.useRouteContext()
          const { requestOrigin } = Route.useLoaderData()
          const clientStack = useMemo(
            () => getStackClient(queryClient, { origin: requestOrigin }),
            [queryClient, requestOrigin],
          )
    ...
    Vite SSR, serialize only the request origin into the parent loader so the
    server provider and hydrated browser stack resolve the same endpoint; keep
