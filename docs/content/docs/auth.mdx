...
The identity and permission facts are inferred from their schemas. A missing rule denies access once authorization is enabled. Use `.allow()` for an explicit unconditional rule.
...
Validated input and trusted facts are deeply readonly in TypeScript and frozen
at runtime before lifecycle hooks run. A `before` hook therefore cannot change
the record that was authorized before `execute` uses it.
