...
explicitly public RC operation does not call `can`. These mappings are a
migration bridge, not a second supported authorization model. An identity-only
RC provider has no authorization policy, so mapped operations retain its prior
allow-all behavior; add `can` to enforce the mapped string permissions. The
bridge is removed with the other RC interfaces in #193. `createServerAuth()`
ignores the mappings and always evaluates the exact schema-backed descriptor.
