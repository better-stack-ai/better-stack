...
BTST's one-rule authorization path keeps one browser-safe rule set as the contract while client and server adapters resolve identity independently. Browser checks only control presentation; backend operations derive trusted facts and enforce the same rule before mutation.
...
The wire request contains the contract version, stable permission id, and schema-validated facts. It intentionally does not send the browser identity as trusted input. The backend validates the request against its contract and resolves identity from its own session:
...
export async function evaluateAuthorization(request: Request) {
  ...
  const allowed = authorization.can(parsed.permission, identity);
