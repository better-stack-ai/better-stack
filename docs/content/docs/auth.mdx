...
Only the schema-validated identity and deployment-trusted API/site origins
...
separate route group with a header-free layout. For public content protected by
local permission rules, pass `initialIdentity={null}` so the server render and
first browser render evaluate those rules as an anonymous reader. Omitting the
snapshot leaves permission gates pending even when the rule allows public reads;
the generated HTML can contain only loading UI instead of the blog content.
The request-aware group keeps per-request identity hydration for its provider
subtree.

An explicit anonymous snapshot skips the initial browser resolver. Mount a
client component inside the static provider to refresh identity after hydration:

```tsx title="app/pages/resolve-static-identity.tsx"
"use client";
...
import { BtstPagesClientLayout } from "@/app/pages/client-layout";
import { ResolveStaticIdentity } from "@/app/pages/resolve-static-identity";
...
export default function StaticPagesLayout({ children }: { children?: ReactNode }) {
  // Static HTML represents an anonymous reader; never embed a user's session.
  return (
...
`/pages`. Both route groups still hydrate the same server-resolved API/site
snapshot, including when `BTST_API_URL` points at a managed backend. The static
group caches only anonymous content. Private routes belong in the request-aware
group, and backend authorization remains required for protected operations.

The refresh can show permission loading UI while the browser resolver runs.
Applications with a shared reactive session can instead pass its validated
identity as the provider snapshot, starting with `null` on the server and first
browser render, then updating it when the session settles or changes. Choose one
session integration to avoid duplicate lookups. With the explicit `refetch`
approach above, call it again after login, logout, or account switching; a mount
effect alone does not subscribe to later session changes.

Omitting `initialIdentity` remains appropriate when deliberately rendering a
loading state until browser authentication resolves. A remote authorization
evaluator also needs a browser decision; an anonymous identity alone does not
make remote permission checks synchronous during static generation.
...
{/* canonical-dx-guard: migration:start reason="removed server namespace example" */}
...
createBackendStack({ auth: serverAuth, plugins: { aiChat } });
```
...
{/* canonical-dx-guard: migration:start reason="removed lifecycle and identity bridge example" */}
