...
Only the validated identity and browser-safe request origin cross the server
boundary. Request headers and the resolved server stack stay server-only. BTST
does not cache authorization results or require framework middleware.
