...
before its fact-dependent descriptor gate can mount. Plugin-owned descriptor
routes that were already public may set `legacyPublic` during the RC migration;
that bypass applies only to string-based providers, while one-rule providers
still evaluate the exact descriptor.
