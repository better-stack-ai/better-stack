...
Lifecycle hooks observe already-validated operation context. They are for transforming domain input, side effects, and telemetry—not routine authorization. Use descriptor rules for access control. Request and internal operations both run the documented hook sequence; raw adapter primitives are lower-level implementation tools and do not promise that lifecycle.
