...
The `onBefore*PageRendered` override callbacks were removed. Built-in protected routes declare resource/action permissions through `ComposedRoute`; implement `auth.can` for client-side gating and keep SSR/backend checks in client loader hooks and backend lifecycle hooks.

### Backend hooks deny by throwing

The compatibility shim for boolean lifecycle-hook returns was removed. Returning `false` no longer denies a request; throw an `Error` from an `onBefore*` hook instead. Hooks that transform input continue to return the transformed object.
