...
Remove shared runtime fields from every built-in plugin factory and override.
Configure API, site, and QueryClient once when creating the client stack, then
pass that resolved stack to the provider:
...
per-plugin precedence or fallback in v3. Resolved built-in plugins use the same
runtime for SSR loaders, metadata, browser hooks, and mutations.
