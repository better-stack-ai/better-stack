...
Earlier v3 prereleases used the past-tense
`onBeforeSubmissionDeleted` spelling. RC3 uses the canonical
`onBeforeDeleteSubmission` action/entity order. Update the name and keep hook
denials exception-based:
...
- Replace Form Builder's prerelease `onBeforeSubmissionDeleted` name with `onBeforeDeleteSubmission`.
