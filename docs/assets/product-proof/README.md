...
The coherent fixture is [dogfood-data.json](../../../e2e/product-proof/dogfood-data.json). It contains no personal data and describes one release-evidence story across Blog, Comments, CMS, Kanban, Form Builder, UI Builder, Media, OpenAPI, Route Docs, and AI Chat. The capture workflow seeds Blog posts, an approved Comment, CMS records, a Kanban board with tasks, a published UI Builder page, and one exact-name Media asset through the registered plugin's direct multipart upload route. It removes all owned fixtures before and after every run, verifies the stored record, filters the generated library route, and requires exactly one visible asset card. Form Builder remains an unsaved interaction; OpenAPI and Route Docs are generated from the registered backend and client stacks. AI Chat requires both an empty authenticated-history API result and the visible empty-sidebar state before it types—but does not send—the fixture prompt.
...
Capture cleans exact Blog slugs, scoped Comments bodies, CMS record slugs, the Kanban board (explicitly deleting its columns and tasks, including on the memory adapter), the UI Builder page slug, and the Media filename before and after the run. Cleanup continues across independent failures and runs after partial setup failures. Forms are never saved and chat prompts are never sent. Unrelated records are preserved. `pnpm test:product-proof` exercises this lifecycle with a controlled API, including pagination and deletion failures.
