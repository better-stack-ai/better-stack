...
// because they preserve Comments-specific optimistic cache behavior. Runtime
// transport and the shared QueryClient still come from the resolved stack.
const commentMutations = commentsResources.comments.mutations;

function useCommentsRuntime() {
	const { api, plugins, queryClient: stackQueryClient } = useStack();
	const pluginApi = plugins?.[COMMENTS_PLUGIN_ID]?.api;
	const headers = pluginApi?.browserHeaders;
	...
	return {
		...
		headers,
		queryClient,
		queries: createCommentsQueryKeys(client),
	};
}
...
export function useComments(
	...
	const { queries, queryClient } = useCommentsRuntime();
 ...
 */
export function useSuspenseComments(params: {
	resourceId?: string;
	resourceType?: string;
	parentId?: string | null;
	status?: "pending" | "approved" | "spam";
 ...
 */
export function useSuspenseModerationComments(params: {
	status?: "pending" | "approved" | "spam";
	limit?: number;
	page?: number;
}) {
 ...
 */
export function useCommentCount(params: {
	resourceId: string;
	resourceType: string;
	status?: "pending" | "approved" | "spam";
}) {
 ...
 */
export function usePostComment(params: {
	resourceId: string;
	resourceType: string;
	currentUserId?: string;
	...
	const { client, headers, queries, queryClient } = useCommentsRuntime();
 ...
 */
export function useToggleLike(params: {
	resourceId: string;
	resourceType: string;
