...
| `.github/workflows/registry.yml` | CI — rebuilds registry and runs E2E on PRs and main |
...
- **Registry not rebuilt after plugin changes** — always run `build-registry` and commit the updated JSON. CI rejects stale output; commit it before final verification. Do not auto-commit or use skip-CI commits after the checks.
