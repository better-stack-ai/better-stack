...
# app/root.tsx and app/routes.ts are generated by shadcn init with content, but
# their E2E patches use an empty baseline (index e69de29). Delete them so the
# @@ -0,0 patches apply cleanly instead of prepending to existing content.
