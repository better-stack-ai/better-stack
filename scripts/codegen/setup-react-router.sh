...
#                       1. bash scripts/codegen/setup-react-router.sh --baseline-only
#                       2. Apply desired changes to codegen-projects/react-router
#                       3. node scripts/codegen/generate-patches-react-router.mjs
#
...
#   5. Applies surgical .patch files from scripts/codegen/patches/react-router/
...
PATCHES="$SCRIPT_DIR/patches/react-router"
...
if [ "$BASELINE_ONLY" = true ]; then
  ...
  echo "    1. Apply desired changes to codegen-projects/react-router"
  echo "    2. node scripts/codegen/generate-patches-react-router.mjs"
  echo ""
  exit 0
fi
...
step "Applying E2E overlay patches from scripts/codegen/patches/react-router/"
cd "$DEST"
