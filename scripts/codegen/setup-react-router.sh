...
mkdir -p "$DEST"
cd "$DEST"
pnpm dlx shadcn@latest init -t react-router --yes
...
step "Applying patches from scripts/codegen/patches/react-router/"
if [ -d "$PATCHES" ]; then
  cp -r "$PATCHES/." "$DEST/"
  success "Patches applied"
else
  die "Patch directory not found: $PATCHES — run after creating patches/react-router/"
fi
...
pkg.scripts["start:e2e"] = "react-router build && NODE_ENV=test react-router-serve ./build/server/index.js";
