...
function DirectoryContent() {
	...
	const { items: categories, isLoading: categoriesLoading } = useContent<
		CMSTypes,
		"category"
	>("category", { limit: 50 });
						...
						{filteredResources.length === 0 ? (
							<div className="text-center py-12 text-neutral-500">
								{search
									...
									: "No resources yet. Create some in the CMS!"}
							</div>
						) : (
							<div className="grid gap-4 sm:grid-cols-2">
								{filteredResources.map((resource) => (
									...
									>
										...
										{resource.parsedData.categoryIds &&
											resource.parsedData.categoryIds.length > 0 && (
												<div className="mt-3 flex gap-1">
													<span className="text-xs px-2 py-0.5 rounded-full bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-400">
														{resource.parsedData.categoryIds.length} categor
														{resource.parsedData.categoryIds.length === 1
															? "y"
															: "ies"}
													</span>
												</div>
