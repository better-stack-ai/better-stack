	...
	return (
		<div className="p-8 max-w-4xl mx-auto">
			...
			<section className="mb-8">
				...
				{!hasContentTypes ? (
					<div
						data-testid="content-types-empty"
						className="p-4 bg-gray-100 dark:bg-gray-800 rounded-lg text-center"
					>
						No content types found. Create content types in the CMS to see them
						here.
					</div>
				) : (
					<div data-testid="content-types-list" className="space-y-2">
						{contentTypes.map((type) => (
							...
							>
								<span className="font-medium" data-testid="content-type-name">
									{type.name}
								</span>
								<span
									className="text-sm text-gray-500"
									data-testid="content-type-count"
								>
			...
			<section>
				<h2 className="text-xl font-semibold mb-4">
					Product Items (Showing:{" "}
					<span data-testid="products-showing">{items.length}</span> / Total:{" "}
					<span data-testid="products-total">{total}</span>)
				</h2>
				{!hasItems ? (
					<div
